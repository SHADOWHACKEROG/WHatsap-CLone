        ...
        <div className="d-flex flex-column flex-grow-1">
                    ...
                    {selectedConversation.messages.map((message, index) => {
                        ...
                        return (
                            <div ref={lastMessage ? setRef : null} key={index} className={`my-1 d-flex flex-column ${message.fromMe ? 'align-self-end' : ''}`}>
                                ...
                                <div className={`hgt text-muted small ${message.fromMe ? 'text-right' : 'text-left'}`}>{message.fromMe ? 'You' : message.senderName}</div>
                            </div>
                        )
                    })}
                </div>
            ...
            <Form onSubmit={handleSubmit}>
                <Form.Group className="m-2">
                    ...
                    <Button type="submit" className="hgb">Send</Button>
                    </InputGroup>
                </Form.Group>
            </Form>
        </div>
